...
def create_datasets(data_path, input_size, rgb=False):
  ...
  train_data_tforms.append(transforms.RandomHorizontalFlip(p=0.6)) 
...
class SimpleNet(nn.Module):
  ...
  def __init__(self, num_classes, droprate=0.5, rgb=False, verbose=False):
    ...
    self.features = nn.Sequential(
      # first layer
      nn.Conv2d(in_channels=in_channels, out_channels=5, kernel_size=9,
        stride=1, padding=0, bias=False), # output volume size = (64-3)/1+1 = 56
      nn.BatchNorm2d(5), 
      # nn.MaxPool2d(kernel_size=7, stride=7, padding=0), 
      nn.MaxPool2d(kernel_size=2, stride=2, padding=0), # (56-2)/2+1 = 28
      nn.ReLU(inplace = True),
    
      # # second layer
      nn.Conv2d(in_channels=5, out_channels=10, kernel_size=4, 
        stride=2, padding=1, bias=False),  # (28 - 4 + 2*1)/2+1 = 14
      nn.BatchNorm2d(10),
      nn.MaxPool2d(kernel_size=2, stride=2, padding=1), # (14-2+2)/2+1=8 
      nn.ReLU(True),

      nn.Dropout(p=0.5)
    )

    # in the above network we should got an output 7*7
...
def custom_part1_trainer(model):
    ...
    paras = {'lr_scheduler': optim.lr_scheduler.StepLR(optimizer, step_size=50, gamma=1),
              'optimizer': optimizer,
...
def create_part2_model(model, num_classes):
  ...
  # new_classifier.append(nn.Dropout())
  # new_classifier.append(nn.Linear(4096, 4096)) 
  # new_classifier.append(nn.ReLU(inplace=True)) 
  # new_classifier.append(nn.Dropout())
  # new_classifier.append(nn.Linear(4096, 4096)) 
  # new_classifier.append(nn.ReLU(inplace = True))
  new_classifier.append(nn.Linear(4096, num_classes)) 
...
def custom_part2_trainer(model):
    ...
    paras = {'lr_scheduler': optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=1),
             'optimizer': optimizer,
