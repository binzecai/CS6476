...
def get_tiny_images(image_paths):
  ...
  for i in range(N):
        argv = image_paths[i]
        image_ori = cv2.resize(load_image_gray(argv), (width, width), interpolation=cv2.INTER_LINEAR)
        image = image_ori.reshape(1, width * width)
        image = image - np.mean(image)
        feats.append((image / np.std(image))[0]) 
  return feats
...
def build_vocabulary(image_paths, vocab_size):
  ...
  N = len(image_paths)
  features = 200 # num of sift features selected from each image
  ...
  cluster_centers = vlfeat.kmeans.kmeans(sift_feats, vocab_size)
...
def get_bags_of_sifts(image_paths, vocab_filename):
  ...
  features = np.zeros((N, vocab_size))
  step_size = 10
  for i in range(N):
      argv = image_paths[i]
      frames, descriptors = vlfeat.sift.dsift(load_image_gray(argv), fast=True, step=step_size)
      ...
      for j in range(assignments.shape[0]):
          bags_of_sifts[0, assignments[j]] += 1
      norm = np.linalg.norm(bags_of_sifts)
      feats.append((bags_of_sifts / (norm + 0.0001))[0])
...
def category(labels):
  result = {}
  for num, item in enumerate(list(set(labels))):
    result[item] = num
  return result
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats,
  ...
  N = len(train_image_feats) # 1500
  d = len(train_image_feats[0]) # 256
  M = len(test_image_feats) # 1500
  D = sklearn_pairwise.pairwise_distances(test_image_feats, train_image_feats)
  k = 1
    
  # map the string labels to integer
  labels_map = category(train_labels) # 15 categories
  train_label = np.zeros((1, len(train_labels)), dtype = np.int32)
  for i in range(len(train_labels)):
    train_label[0, i] = labels_map[train_labels[i]]
    
  index = np.argsort(D, axis = 1) # increasing order
  knn_labels = np.zeros((D.shape[0], k), dtype=np.int32)
  for i in range(D.shape[0]): # how many feats
    for j in range(k):
        knn_labels[i, j] = train_label[0, index[i, j]] 
    
  for i in range(knn_labels.shape[0]):
    curr = np.bincount(knn_labels[i, :])
    curr = np.argmax(curr)
    test_labels.append(get_keys(labels_map, curr)[0])
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
  ...
  for item in svms:
      ...
      Y = np.zeros((len(train_labels), 1))
      for i in range(len(train_labels)):
          if item == train_labels[i]:
              Y[i] = 1
          else:
              Y[i] = 0
      svms[item].fit(train_image_feats, Y.ravel())
